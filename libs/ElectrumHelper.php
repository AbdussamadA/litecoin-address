...
#It was modified to work with litecoin. It comes with no warranty.
...
spl_autoload_register( 
    function( $full_obj ) {
        ...
        $obj = end( $arr );
        if( strpos( $full_obj, "LitecoinAddress" ) !== false && preg_match( '/^[a-z0-9_]+$/i', $obj ) ) {
            @include "$obj.php";
        }
    }
);
...
class ElectrumHelper
{
    ...
    public static function mpk_to_lc_address($mpk, $index, $version, $is_for_change = false) {
        if ($version == self::V1) {
        ...
        return self::pubkey_to_lc_address($pubkey);
    }
    ...
    public static function pubkey_to_lc_address($pubkey) {
        return self::hash_160_to_lc_address(self::hash_160(hex2bin($pubkey)));
    }

    public static function hash_160_to_lc_address($h160, $addrtype = 48) {
